...
async function run() {
    console.log('--- Inspecting interdicoes table ---');
    const { data, error } = await supabase.from('interdicoes').select('*').limit(1);

    let output = '';
    if (error) {
        output = 'Error fetching interdicoes: ' + JSON.stringify(error, null, 2);
    } else if (data && data.length > 0) {
        output += 'Columns found: ' + Object.keys(data[0]).join(', ') + '\n\n';
        output += 'Sample record:\n' + JSON.stringify(data[0], null, 2);
    } else {
        output = 'No records found in interdicoes table.';
    }

    fs.writeFileSync('interdicoes_schema_inspect.txt', output, 'utf8');
    console.log('Results written to interdicoes_schema_inspect.txt');
}
