# scripts/convert_all_originals_smart.py
import os
import re
import json
import shutil
...
PROBLEMATIC_FILES = ['OF009-2017 EDP.doc']

def run_conversion():
    temp_dir = os.path.abspath('temp_oficios_original_v6')
    ...
    subprocess.run(['tar', '-xf', 'Oficios_Legado.rar', '-C', 'temp_oficios_original_v6'], capture_output=True)
    ...
    print("Iniciando conversão automatizada preservando 100% dos cabeçalhos originais...")
        ...
        for key in sorted_keys:
            ...
            if not success and data["doc_file"] and os.path.exists(data["doc_file"]):
                ...
                if raw_name in PROBLEMATIC_FILES:
                    ...
                    try:
                        print(f"[{item_id}] Convertendo Word COM: {os.path.basename(doc_abs)}")
            ...
            # 3. Fallback se o arquivo original for problemático/corrompido
            if not success:
                item_temp = {
    ...
    print(f"\nResumo da Conversão Completa dos Arquivos Originais:")
    print(f" - PDFs originais copiados: {copied_count}")
    print(f" - Documentos Word originais convertidos diretamente: {converted_count}")
    print(f" - PDFs de suporte (fallback para arquivos com aviso): {fallback_count}")
