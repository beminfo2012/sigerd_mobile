# scripts/convert_all_originals_final.py
import os
import re
import json
import shutil
...
PROBLEMATIC_FILES = ['OF009-2017 EDP.doc', 'OF008-2017 Bombeiros Voluntários.doc']
...
def run_final_pipeline():
    kill_word()
    temp_dir = os.path.abspath('temp_oficios_original_final_v2')
    ...
    subprocess.run(['tar', '-xf', 'Oficios_Legado.rar', '-C', 'temp_oficios_original_final_v2'], capture_output=True)
    ...
    # Normalização de nomes e extensões duplas
    print("Step 2: Normalizando nomes de arquivos e extensões no acervo...")
    for root, dirs, files in os.walk(rar_base):
        for f in files:
            if f.endswith('.docx.docx'):
                new_f = f[:-5] # remove o .docx extra
                old_p = os.path.join(root, f)
                new_p = os.path.join(root, new_f)
                try: os.rename(old_p, new_p)
                except: pass

    print("Step 3: Convertendo documentos originais via Word COM (preservando 100% dos cabeçalhos)...")
    ...
    for year_str in years:
        ...
        for key in sorted_keys:
            ...
            # 2. Converter via Word COM direto se não for arquivo problemático
            if not success and data["doc_file"] and os.path.exists(data["doc_file"]):
                raw_name = data.get("raw_filename", "")
                if raw_name in PROBLEMATIC_FILES:
                    print(f"Pulando arquivo problemático para Word COM: {raw_name}")
                else:
                    ...
                    try:
                        ...
                        doc.SaveAs(FileName=pdf_abs, FileFormat=17) # wdFormatPDF
            ...
            # 3. Fallback se o arquivo original estiver corrompido/bloqueado
            if not success:
                item_temp = {
    ...
    print(f"\nStep 4: Resumo da Conversão dos Arquivos Originais:")
    ...
    print(f" - PDFs de suporte (fallback para corrompidos): {fallback_count}")
    print(f" - Total de arquivos PDF gerados no acervo: {len(legacy_items)}")

    print("\nStep 5: Executando upload para o Supabase Storage Bucket...")
