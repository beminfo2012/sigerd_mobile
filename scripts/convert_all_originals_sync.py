# scripts/convert_all_originals_sync.py
import os
import re
import json
import shutil
...
PROBLEMATIC_DOCS = ['OF009-2017 EDP.doc', 'OF008-2017 Bombeiros Voluntários.doc']
...
def run_sync_pipeline():
    kill_word()
    temp_dir = os.path.abspath('temp_oficios_sync')
    ...
    subprocess.run(['tar', '-xf', 'Oficios_Legado.rar', '-C', 'temp_oficios_sync'], capture_output=True)
    ...
    for year_str in years:
        ...
        for key in sorted_keys:
            ...
            # 2. Converter via Word COM direto
            if not success and data["doc_file"] and os.path.exists(data["doc_file"]):
                raw_name = data.get("raw_filename", "")
                if raw_name in PROBLEMATIC_DOCS:
                    pass
                else:
                    doc_abs = os.path.abspath(data["doc_file"])
                    pdf_abs = os.path.abspath(target_pdf_path)
                    try:
                        doc = word.Documents.Open(FileName=doc_abs, ConfirmConversions=False, ReadOnly=True, AddToRecentFiles=False)
                        doc.SaveAs(FileName=pdf_abs, FileFormat=17)
                        doc.Close(SaveChanges=0)
                        converted_doc_count += 1
                        success = True
                    except Exception as e:
                        print(f"Erro em {os.path.basename(doc_abs)}: {e}")

            # 3. Fallback para arquivo corrompido/inexistente
            if not success:
                item_temp = {
    ...
    print(f"\nStep 2: Resumo do Processamento Final:")
    print(f" - PDFs originais copiados: {copied_pdf_count}")
    print(f" - Documentos Word originais convertidos diretamente: {converted_doc_count}")
    print(f" - PDFs de suporte (fallback): {fallback_count}")
    print(f" - Total de arquivos PDF gerados no acervo: {len(legacy_items)}")

    # Step 3: Upload para o Supabase Storage Bucket e atualização das URLs
    print("\nStep 3: Executando upload para o Supabase Storage Bucket...")
