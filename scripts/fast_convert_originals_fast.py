# scripts/fast_convert_originals_fast.py
import os
import re
import json
import shutil
...
def run_ultra_fast_conversion():
    kill_word()
    temp_dir = os.path.abspath('temp_oficios_original_fast')
    if os.path.exists(temp_dir):
        try: shutil.rmtree(temp_dir)
        except: pass
    os.makedirs(temp_dir, exist_ok=True)

    print("Step 1: Extraindo acervo original do RAR...")
    subprocess.run(['tar', '-xf', 'Oficios_Legado.rar', '-C', 'temp_oficios_original_fast'], capture_output=True)
    ...
    for root, dirs, files in os.walk(rar_base):
        for f in files:
            if f.endswith('.docx.docx'):
                new_f = f[:-5]
                try: os.rename(os.path.join(root, f), os.path.join(root, new_f))
                except: pass

    print("Step 2: Inicializando Word COM com aceleração máxima (ScreenUpdating = False)...")
    ...
    for year_str in years:
        ...
        for key in sorted_keys:
            ...
            # 2. Converter via Word COM ultrarrápido preservando 100% dos cabeçalhos originais
    ...
    print(f"\nStep 3: Resumo da Conversão de Arquivos Originais:")
    ...
    print(f" - Total de arquivos PDF no acervo: {len(legacy_items)}")
