...
const RiskDashboard = ({ hideHeader = false }) => {
    ...
    return (
        <div className={hideHeader ? "" : "bg-slate-50 min-h-screen pb-24 font-sans text-slate-800"}>
            {/* Header */}
            {!hideHeader && (
            ...
            {hideHeader && (
                <div className="px-6 pt-6 flex justify-end">
                    <div className="flex gap-2">
                        <button
                            onClick={() => setShowFilters(!showFilters)}
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${showFilters || selectedBairro !== 'Todos' || selectedRisk !== 'Todos' || selectedSeverity !== 'Todos' ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700'}`}
                        >
                    ...
                    <div className="h-[450px] w-full rounded-b-[30px] overflow-hidden relative z-0">
                        {filteredData.length > 0 && filteredData[0].lat ? (
                            ...
                            >
                                ...
                                <LimiteSMJLayer keyId="limite-smj-risk" />
