    ...
    return (
        <div className="bg-slate-50 min-h-screen pb-24 font-sans text-slate-800">
            ...
            <div className="max-w-4xl mx-auto">
                ...
                {/* DOCUMENTOS OFICIAIS (APENAS DEFESA CIVIL) */}
                {['Admin', 'Coordenador', 'Agente de Defesa Civil'].includes(user?.role) && (
                    <>
                        <SectionHeader
                            icon={FileStack}
                            title="Documentos Oficiais (Decreto, Ofício e Parecer)"
                            isOpen={openSections.documentos}
                            onToggle={() => setOpenSections(prev => ({ ...prev, documentos: !prev.documentos }))}
                            color="blue"
                        />
                            ...
                            <div className="p-4 bg-white space-y-6 animate-in slide-in-from-top-2 duration-300">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col gap-3">
                                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Configurações Gerais</h4>
                                        <div>
                                            <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Nome do Prefeito</label>
                                            <input
                                                type="text"
                                                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold"
                                                value={formData.data.metadata_oficial.nome_prefeito}
                                                onChange={(e) => updateMetadataOficial('nome_prefeito', e.target.value)}
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Cód. Protocolo S2ID</label>
                                            <input
                                                type="text"
                                                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold"
                                                placeholder="UF-F-0000000-00000-00000000"
                                                value={formData.data.metadata_oficial.protocolo_s2id}
                                                onChange={(e) => updateMetadataOficial('protocolo_s2id', e.target.value)}
                                            />
                                        </div>
                                    </div>

                                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col gap-3">
                                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Dados do Decreto</h4>
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Nº Decreto</label>
                                                <input
                                                    type="text"
                                                    className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold"
                                                    value={formData.data.metadata_oficial.decreto_numero}
                                                    onChange={(e) => updateMetadataOficial('decreto_numero', e.target.value)}
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Vigência (Dias)</label>
                                                <input
                                                    type="number"
                                                    className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold"
                                                    value={formData.data.metadata_oficial.decreto_vigencia}
                                                    onChange={(e) => updateMetadataOficial('decreto_vigencia', e.target.value)}
                                                />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Data Publicação D.O.</label>
                                            <input
                                                type="text"
                                                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold"
                                                placeholder="Ex: Edição 2432 de 12/02/2026"
                                                value={formData.data.metadata_oficial.diario_oficial_info}
                                                onChange={(e) => updateMetadataOficial('diario_oficial_info', e.target.value)}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="bg-blue-50/30 p-4 rounded-2xl border border-blue-100/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                                        <div className="text-left flex-1">
                                            <p className="text-[10px] font-black text-blue-700 uppercase tracking-tighter">1. Decreto Municipal</p>
                                            <p className="text-[9px] text-blue-600/70 italic">Gera o documento legal de declaração de SE/ECP baseado no FIDE.</p>
                                        </div>
                                        <button
                                            onClick={() => generateS2idDoc('decreto', { ...formData.data, ...formData.data.metadata_oficial })}
                                            className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md active:scale-95 transition-all text-center"
                                        >
                                            Gerar Decreto .DOCX
                                        </button>
                                    </div>

                                    <div className="bg-indigo-50/30 p-4 rounded-2xl border border-indigo-100/50 flex flex-col gap-4">
                                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                            <div className="text-left flex-1">
                                                <p className="text-[10px] font-black text-indigo-700 uppercase tracking-tighter">2. Ofício de Requerimento</p>
                                                <p className="text-[9px] text-indigo-600/70 italic">Solicita formalmente o reconhecimento da União.</p>
                                            </div>
                                            <button
                                                onClick={() => generateS2idDoc('oficio', { ...formData.data, ...formData.data.metadata_oficial })}
                                                className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md active:scale-95 transition-all text-center"
                                            >
                                                Gerar Ofício .DOCX
                                            </button>
                                        </div>
                                        <div className="space-y-2">
                                            <label className="block text-[8px] font-black text-indigo-400 uppercase ml-1">Justificativa da Necessidade de Reconhecimento</label>
                                            <textarea
                                                className="w-full px-4 py-3 bg-white border border-indigo-100 rounded-xl text-xs min-h-[80px] focus:ring-2 focus:ring-indigo-500/10 outline-none"
                                                placeholder="Descreva a motivação da necessidade do reconhecimento federal e os benefícios pleiteados..."
                                                value={formData.data.metadata_oficial.justificativa_federal}
                                                onChange={(e) => updateMetadataOficial('justificativa_federal', e.target.value)}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                                        <div className="text-left flex-1">
                                            <p className="text-[10px] font-black text-slate-700 uppercase tracking-tighter">3. Parecer Técnico</p>
                                            <p className="text-[9px] text-slate-600/70 italic">Fundamentação técnica resumindo danos e efeitos.</p>
                                        </div>
                                        <button
                                            onClick={() => generateS2idDoc('parecer', { ...formData.data, ...formData.data.metadata_oficial })}
                                            className="px-6 py-2.5 bg-slate-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md active:scale-95 transition-all text-center"
                                        >
                                            Gerar Parecer .DOCX
                                        </button>
                                    </div>
