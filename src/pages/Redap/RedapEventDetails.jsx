        ...
        <div className="bg-slate-50 min-h-screen pb-24 font-sans text-slate-800">
            ...
            <header className="bg-white border-b border-slate-200 px-4 h-16 flex items-center justify-between sticky top-0 z-20 shadow-sm">
                <div className="flex items-center gap-3">
                    <button onClick={() => navigate('/redap')} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-600">
                        <ArrowLeft size={20} />
                    </button>
                    <div>
                        <h1 className="text-base font-black text-slate-800 leading-tight">Painel do Desastre</h1>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-tight flex items-center gap-1">
                            <Shield size={10} className="text-blue-500" /> {event?.cobrade}
                        </p>
                    </div>
                </div>
                <button
            ...
            <main className="p-4 space-y-6 max-w-[1600px] mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="md:col-span-2 bg-white rounded-[2.5rem] p-7 shadow-sm border border-slate-100 flex items-center gap-6">
                        <div className="bg-blue-600 p-5 rounded-[2rem] text-white shadow-xl shadow-blue-100">
                            <TrendingUp size={32} />
                        </div>
                        <div className="flex-1">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Impacto Financeiro Estimado</p>
                    ...
                    <div className="bg-slate-900 rounded-[2.5rem] p-7 text-white shadow-xl flex items-center justify-between">
                        <div>
                            <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Ações do Gestor</p>
                            <h3 className="text-lg font-black uppercase leading-tight">Fechar Pasta<br/>Gerar FIDE</h3>
                        </div>
                        <button 
                            className="bg-blue-600 p-4 rounded-2xl shadow-lg active:scale-95 transition-all hover:bg-blue-500"
                    ...
                    <div className="bg-emerald-600 rounded-[2.5rem] p-7 text-white shadow-xl flex items-center justify-between">
                        <div>
                            <p className="text-[10px] font-black text-emerald-200 uppercase tracking-widest mb-1">Visualização Geográfica</p>
                            <h3 className="text-lg font-black uppercase leading-tight">Mapa de<br/>Danos</h3>
                        </div>
                        <button className="bg-emerald-700 p-4 rounded-2xl shadow-lg active:scale-95 transition-all hover:bg-emerald-800">
                            <MapIcon size={24} />
                        </button>
                    </div>
                </div>
                        ...
                        <div key={col.id} className="space-y-4">
                            <div className="flex items-center justify-between px-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest">{col.title}</h3>
                                </div>
                            ...
                            <div className="space-y-4 min-h-[500px]">
                                {registrations.filter(r => r.status_validacao === col.id).map(reg => (
                                    <div 
                                        key={reg.id} 
                                        className="bg-white rounded-[2rem] p-5 shadow-sm border border-slate-100 hover:shadow-xl hover:border-blue-100 transition-all group"
                                    >
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                {col.id !== 'Enviado' && (
                                                    <button 
                                                        ...
                                                        className="p-1.5 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-600 hover:text-white transition-all"
                                                    >
                                                        <Clock size={14} />
                                                    </button>
                                                )}
                                        ...
                                        <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                                            <div className="flex -space-x-2">
                                                ...
                                                {reg.fotos?.length > 3 && (
                                                    <div className="w-6 h-6 rounded-full border-2 border-white bg-slate-800 text-[8px] flex items-center justify-center text-white font-bold">
                                                        +{reg.fotos.length - 3}
                                                    </div>
                                                )}
                                            </div>
                                            <button 
                                                ...
                                                className="text-[9px] font-black text-slate-400 hover:text-blue-600 uppercase tracking-widest flex items-center gap-1"
                                            >
                                ...
                                {registrations.filter(r => r.status_validacao === col.id).length === 0 && (
                                    <div className="py-12 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-[2.5rem] bg-slate-50/50">
                                        ...
                                        <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Coluna Vazia</p>
                                    </div>
                                )}
                            </div>
                        </div>
