                ...
                <div className="flex items-center justify-between mb-8 px-4">
                    {[1, 2, 3].map((s) => (
                        <div key={s} className="flex flex-col items-center gap-2 relative z-10">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step >= s ? 'bg-[#2a5299] text-white shadow-lg scale-110' : 'bg-white dark:bg-slate-900 text-slate-300 border border-slate-200 dark:border-slate-700'
                                }`}>
                                {s}
                            </div>
                    ...
                    <div className="absolute left-0 right-0 top-6 h-0.5 bg-slate-200 dark:bg-slate-700 -z-0 max-w-3xl mx-auto px-8 hidden md:block" />
                </div>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                className="bg-white dark:bg-slate-900 shadow-sm pl-10"
                                value={searchQuery}
                        ...
                        {selectedItems.length > 0 && (
                            <div className="flex items-center justify-between bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-900/50">
                        ...
                        {isLoadingInitial ? (
                            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                                ...
                                <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Carregando estoque...</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 gap-3">
                                {centralStock.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${isSelected ? 'bg-[#2a5299] text-white scale-105' : 'bg-slate-50 dark:bg-slate-800/50 text-slate-400 group-hover:bg-white dark:bg-slate-900'}`}>
                                                        {isSelected ? <CheckCircle2 size={24} /> : <Package size={24} />}
                                                    </div>
                                                    <div>
                                                        <h3 className="font-bold text-slate-800 dark:text-slate-100 leading-tight">{item.item_name}</h3>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{item.category}</span>
                ...
                {step === 2 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="space-y-4">
                            <h3 className="font-bold text-slate-800 dark:text-slate-100">1. Definir Destino</h3>
                            <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex">
                                <button
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${destinationType === 'SHELTER' ? 'bg-white dark:bg-slate-900 shadow-sm text-[#2a5299]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'
                                        }`}
                                ...
                                <button
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${destinationType === 'PERSON' ? 'bg-white dark:bg-slate-900 shadow-sm text-[#2a5299]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'
                                        }`}
                            ...
                            {destinationType === 'SHELTER' ? (
                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
                                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">Abrigo de Destino</label>
                                    <select
                                        className="w-full p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 font-semibold text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-[#2a5299]/20 outline-none"
                        ...
                        <div className="border-t border-slate-200 dark:border-slate-700 pt-6 space-y-4">
                            <h3 className="font-bold text-slate-800 dark:text-slate-100 flex justify-between items-center">
                                2. Definir Quantidades
                                <span className="text-xs font-normal text-slate-500 dark:text-slate-400">{selectedItems.length} itens</span>
                            </h3>

                            {selectedItems.map((item, idx) => (
                                <div key={idx} className="bg-white dark:bg-slate-900 border rounded-xl p-4 shadow-sm flex flex-col md:flex-row md:items-center gap-4">
                                    <div className="flex items-center gap-3 flex-1">
                                        <div className="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-[#2a5299] shrink-0">
                                            <Package size={20} />
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800 dark:text-slate-100 text-sm">{item.item_name}</h4>
                                            <p className="text-xs text-slate-500 dark:text-slate-400">Estoque: {item.quantity} {item.unit}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 w-full md:w-auto">
                                        ...
                                        <button
                                            onClick={() => toggleItemSelection(item)}
                                            className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 dark:bg-red-900/20 rounded-lg transition-colors ml-2"
                                        >
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                ...
                {step === 3 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        <Card className="p-6 text-center space-y-4">
                            <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/20 rounded-full flex items-center justify-center mx-auto text-[#2a5299]">
                                <Truck size={32} />
                            </div>
                            <div>
                                <h2 className="text-xl font-black text-slate-800 dark:text-slate-100">Confirmar Distribuição?</h2>
                                <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Você está prestes a enviar {selectedItems.length} itens.</p>
                            </div>

                            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 text-left space-y-3 text-sm">
                                ...
                                <div className="space-y-2 pt-2">
                                    {selectedItems.map((item, idx) => (
                                        <div key={idx} className="flex justify-between items-center text-xs">
                                            <span className="text-slate-600 dark:text-slate-300">{item.item_name}</span>
