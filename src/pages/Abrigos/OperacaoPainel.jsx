...
export default function OperacaoPainel() {
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-800/50 dark:bg-slate-950">
    ...
    if (!operacao) {
        return (
            <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/50 dark:bg-slate-950 min-h-screen">
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-800/50 dark:bg-slate-950 min-h-screen pb-24 text-slate-800 dark:text-slate-100 font-sans">
            <header className="bg-white dark:bg-slate-900/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-30 border-b border-slate-200 dark:border-slate-700 dark:border-slate-800 px-4 py-3">
                <div className="max-w-6xl mx-auto flex items-center gap-4">
                    <button onClick={() => navigate(-1)} className="p-2 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-600 dark:text-slate-300 dark:text-slate-400">
                    ...
                    <div className="flex flex-1 items-center justify-between">
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                {isEncerrada ? (
                                    <span className="text-[9px] px-2 py-0.5 rounded-full font-bold bg-red-100 dark:bg-red-900/30 text-red-600 dark:bg-red-900/30 dark:text-red-400">ENCERRADA</span>
                                ) : (
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate max-w-sm">{operacao.nome}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            {operacao.status === 'aberta' && (
                                <button 
                                    className="px-3 py-1.5 text-xs bg-red-50 dark:bg-red-900/20 text-red-600 hover:bg-red-100 dark:bg-red-900/30 rounded-lg font-bold transition-colors"
                            ...
                            <button 
                                className="px-3 py-1.5 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-100 rounded-lg font-bold transition-colors flex items-center gap-1"
            ...
            {isEncerrada && (
                <div className="bg-red-50 dark:bg-red-900/20 dark:bg-red-900/20 border-b border-red-100 dark:border-red-900/50 p-2 text-center text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-widest flex items-center justify-center gap-2">
            ...
            <main className="max-w-6xl mx-auto px-4 py-6">
                <div className="flex overflow-x-auto custom-scrollbar gap-2 mb-6 pb-2">
                    {tabs.map(tab => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-colors flex-shrink-0 ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-sm' : 'bg-white dark:bg-slate-900 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-800/50 dark:hover:bg-slate-700'}`}
                        >
                            <tab.icon size={16} />
                            {tab.label}
                        </button>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[28px] border border-slate-200 dark:border-slate-700 dark:border-slate-800 shadow-sm p-6">
                    {activeTab === 'diario' && (
                        <div className="flex flex-col h-[600px]">
                            ...
                            <div className="flex-1 overflow-y-auto space-y-4 pr-2 mb-4">
                                {diario.length === 0 ? (
                                    <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-10">Nenhum evento registrado.</p>
                                ) : (
                                    diario.map((evento, i) => (
                                        <div key={i} className="flex gap-4">
                                            <div className="flex flex-col items-center">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-lg shadow-sm border ${evento.origem === 'automatico' ? 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800/50 text-blue-500'}`}>
                                                    {evento.origem === 'automatico' ? '🤖' : '✍️'}
                                                </div>
                                                {i !== diario.length - 1 && <div className="w-px h-full bg-slate-200 dark:bg-slate-700 dark:bg-slate-700 my-1"></div>}
                                            </div>
                                            <div className="flex-1 pb-4">
                                                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 dark:border-slate-700">
                                                    <div className="flex items-center justify-between mb-2">
                                                        ...
                                                        {evento.profiles && evento.origem !== 'automatico' && (
                                                            <span className="text-[10px] bg-slate-200 dark:bg-slate-700 dark:bg-slate-700 px-2 py-0.5 rounded-full font-bold">{evento.profiles.nome}</span>
                                                        )}
                                                    </div>
                                                    <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{evento.descricao}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))
                            ...
                            {!isEncerrada ? (
                                ...
                                <div className="mt-auto bg-slate-100 dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 text-center">
                                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Operação encerrada — somente leitura</p>
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                                    <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Início da Operação</span>
                                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 dark:text-slate-200 mt-1">{new Date(operacao.data_hora_inicio).toLocaleString('pt-BR')}</p>
                                </div>
                                <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                                    <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Encerramento</span>
                                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 dark:text-slate-200 mt-1">{operacao.data_hora_encerramento ? new Date(operacao.data_hora_encerramento).toLocaleString('pt-BR') : 'Em andamento'}</p>
                                </div>
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-200 dark:border-blue-800/50 dark:border-blue-800">
                                ...
                                <div className={`p-4 rounded-xl border ${isEncerrada ? 'bg-red-50 dark:bg-red-900/20 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-emerald-50 dark:bg-emerald-900/20 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800'}`}>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                                ...
                                <div className="bg-emerald-50 dark:bg-emerald-900/20 dark:bg-emerald-900/20 p-4 rounded-xl border border-emerald-200 dark:border-emerald-800 text-center flex flex-col items-center justify-center">
                                ...
                                <div className="bg-amber-50 dark:bg-amber-900/20 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-200 dark:border-amber-800 text-center flex flex-col items-center justify-center">
                            ...
                            {operacao.descricao && (
                                <div className="bg-white dark:bg-slate-900 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 rounded-xl">
                                    <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Contexto / Descrição Inicial</span>
                                    <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-slate-300 mt-2 whitespace-pre-wrap leading-relaxed">{operacao.descricao}</p>
                                </div>
                            )}

                            {operacao.parecer_final && (
                                <div className="bg-amber-50 dark:bg-amber-900/20 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 p-4 rounded-xl">
                    ...
                    {activeTab === 'abrigos' && (
                        <div className="flex flex-col">
                            ...
                            {abrigos.length === 0 ? (
                                <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-10">Nenhum abrigo vinculado a esta operação.</p>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {abrigos.map(a => (
                                        <div key={a.id} className="bg-slate-50 dark:bg-slate-800/50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-800 dark:border-slate-700">
                                            ...
                                            <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400 mt-2">
                                                ...
                                                <span className={`px-2 py-0.5 rounded-full font-bold uppercase ${a.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200'}`}>
                    ...
                    {activeTab === 'estoque' && (
                        <div className="flex flex-col">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div className="bg-emerald-50 dark:bg-emerald-900/20 dark:bg-emerald-900/20 p-4 rounded-xl flex items-center gap-4">
                                ...
                                <div className="bg-amber-50 dark:bg-amber-900/20 dark:bg-amber-900/20 p-4 rounded-xl flex items-center gap-4">
                            ...
                            <h4 className="font-bold text-sm text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">Material Consumido / Distribuído</h4>
                            {(() => {
                                ...
                                if (consumedList.length === 0) {
                                    return <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-6">Nenhum consumo registrado durante o período da operação.</p>;
                                }

                                return (
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                        {consumedList.map((item, idx) => (
                                            <div key={idx} className="bg-white dark:bg-slate-900 dark:bg-slate-800 p-3 rounded-lg border border-slate-100 dark:border-slate-800 dark:border-slate-700 shadow-sm flex justify-between items-center">
                                                <div className="min-w-0">
                                                    <p className="font-bold text-slate-800 dark:text-slate-100 truncate text-sm">{item.item_name}</p>
                                                    <p className="text-xs text-slate-500 dark:text-slate-400 capitalize">{item.category || 'Geral'}</p>
                                                </div>
